...
document.addEventListener('DOMContentLoaded', () => {
    ...
    closeBtn.onclick = () => galleryOverlay.classList.remove('active');

    // 2. Load Gallery from Storage
    function renderGallery() {
        ...
        styles.forEach((src, index) => {
            ...
            item.innerHTML = `
                <img src="${src}" onclick="expandImage('${src}')">
                <button class="del-img-btn" onclick="deleteImage(${index})">&times;</button>
            `;
    ...
    uploadInput.onchange = (e) => {
        ...
        if (file) {
            ...
            reader.onload = (event) => saveToStorage(event.target.result);
            reader.readAsDataURL(file);
        }
    };

    // 4. Handle Pinterest/Web Link
    pinBtn.onclick = () => {
        const url = prompt("Paste Pinterest Image Address (Right-click image > Copy Image Address):");
        if (url && url.trim() !== "") saveToStorage(url);
    };

    function saveToStorage(source) {
        const styles = JSON.parse(localStorage.getItem(STORAGE_KEY)) || [];
        styles.unshift(source); // Add to beginning
    ...
    window.deleteImage = (index) => {
        if(confirm("Remove this style from gallery?")) {
